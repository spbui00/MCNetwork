...
std::tuple<std::vector<double>, std::vector<double>, std::vector<double>> const
JobManager::runControlVoltagesSetup(std::vector<double> const &voltages) {
    ...
    for (int i = 0; i < nJobs; i++) {
        ...
        job.stepsPerTask = parameterStorage->parameters.at("calcCurrentSteps") /
                           job.tasksPerJob;
        ...
        for (int j = 0; j < nInputElectrodes; j++) {
            job.voltages[inputElectrodes[j]] =
    ...
    if (parameterStorage->parameters.at("threads") > 1) {
        ...
        for (int k = 0; k < parameterStorage->parameters.at("threads"); k++) {
            threads.push_back(std::thread(&JobManager::handleJobList,
                                          std::ref(jobs), systems[k],
                                          std::ref(jobSearchMutex)));
        }
        for (int k = 0; k < parameterStorage->parameters.at("threads"); k++) {
            threads[k].join();
        }
    ...
    while (true) {
        ...
        if (mostTasks > 1) {
            ...
            if (bestJob->tasksToGo + 1 == bestJob->tasksPerJob) {
                // first thread in job, so potential and equilibrium state have
                // to be computed. therefor first lock the job.
                ...
                searchMutex.unlock(); // job dont has to be locked, so other
                                      // jobs can start searching
            ...
            // "<<charge<<std::endl;
            bestJob->resultCurrent +=
                *(system->outputCurrentCounter) / system->time;
            bestJob->resultCurrentUncert +=
                std::pow(*(system->outputCurrentCounter) / system->time,
                         2); // storing current**2 here
            system->reset();

            // std::cout<<"current "<<bestJob->resultCurrent<<" current counter
            // "<<
            // *(system->outputCurrentCounter) <<" time
            ...
            while (true) {
                ...
                if (bestJob->tasksToGo > 0) {
                    ...
                    bestJob->resultCurrent +=
                        *(system->outputCurrentCounter) / system->time;
                    bestJob->resultCurrentUncert += std::pow(
                        *(system->outputCurrentCounter) / system->time, 2);
                    bestJob->resultTime += system->time;
                    system->reset();
                } else {
                    if (bestJob->threadNumber == 1) {
                        // finish job
                        bestJob->resultCurrentUncert =
                            std::sqrt(bestJob->resultCurrentUncert -
                                      bestJob->resultCurrent *
                                          bestJob->resultCurrent /
                                          bestJob->tasksPerJob) /
                            bestJob->tasksPerJob;
                        ...
                        std::cout << " time: " << bestJob->resultTime;
                        std::cout << std::endl;
                    }
                    bestJob->threadNumber -= 1;
                    // std::cout<<"job done, job: "<<bestJob->ID<<" threads
                    // left:
