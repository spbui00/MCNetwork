...
private:
    ...
    std::shared_ptr< std::unordered_map<std::vector<bool>,std::shared_ptr<std::vector<double>>>> konwnPartRatesSumList; /*!< map of lists of accumulated rates for binary search, to store known states. only used in storing mode. */
