 ...
 */
DataFile::DataFile(std::string filename, bool createNew): filename(filename) {
...
std::unique_ptr<std::vector<double>> DataFile::readFullDataset(std::string datasetName){
...
std::unique_ptr<std::vector<double>> DataFile::readDatasetSlice(std::string datasetName, int index){
    ...
    //get dataspace
    DataSpace dataspace = dataset.getSpace();
    offset[indexMap[datasetName]][0] = index; // its no problem to change the (globaly used) offset here, because its resetted in addData() anyway
    dataspace.selectHyperslab( H5S_SELECT_SET, dimsf[indexMap[datasetName]], offset[indexMap[datasetName]] ); //dimsf is already size of one slice
