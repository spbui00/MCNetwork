...
# for fileNumber in [1]:
    ...
    trajectoriesSortedByStartEnd = [[[] for j in range(len(electrodes))] for i in range(len(electrodes)) ]
    trajectories = []
    hops = 20000
    IDs = {}
    hitID = 0
    ...
    for i in range(len(trajectories)):
        if trajectories[i][0][0] >= parameters["acceptorNumber"]:
            if trajectories[i][-1][1] >= parameters["acceptorNumber"]:
                trajectoriesSortedByStartEnd[trajectories[i][0][0] - int(parameters["acceptorNumber"])][trajectories[i][-1][1] - int(parameters["acceptorNumber"])].append(trajectories[i])
    ...
    for k in range(len(electrodes)):
        ...
        electodePlotWidth = 8
        for i in range(len(electrodes)):
            if   i == parameters["outputElectrode"]: col= "blue"
            elif i == parameters["inputElectrode1"]:
                if fileNumber in [3,4]: col = "red"
                else:                   col = "rosybrown"
            elif i == parameters["inputElectrode2"]:
                if fileNumber in [2,4]: col = "red"
                else:                   col = "rosybrown"
            else:                                    col= "green"
            ...
            if parameters["geometry"] == "rect":
                ...
                ax.add_artist(plt.Rectangle(xy,electodePlotWidth, parameters["electrodeWidth"], angle=angle, fc=col,ec=col,zorder = -1))
            elif parameters["geometry"] == "circle":
                electrodeWidth = parameters["electrodeWidth"]/(parameters["radius"]*2*np.pi)*360 #in degrees
                ax.add_artist(Wedge((0,0),parameters["radius"]+electodePlotWidth/2,electrodes[i][0]-electrodeWidth/2,electrodes[i][0]+electrodeWidth/2, width = electodePlotWidth, fc=col,ec=col,zorder = -1))
        ...
        for l in range(len(electrodes)):
            ...
            for i in range(len(trajectories)):
                for j in  range(len(trajectories[i])):
                    ...
                    if hoppingSite1 >= parameters["acceptorNumber"]: x1, y1 = electrodePositions[hoppingSite1-int(parameters["acceptorNumber"])][0], electrodePositions[hoppingSite1-int(parameters["acceptorNumber"])][1]
                    else                                           : x1, y1 = acceptorPos[hoppingSite1,0],                                           acceptorPos[hoppingSite1,1]

                    if hoppingSite2 >= parameters["acceptorNumber"]: x2, y2 = electrodePositions[hoppingSite2-int(parameters["acceptorNumber"])][0], electrodePositions[hoppingSite2-int(parameters["acceptorNumber"])][1]
                    else                                           : x2, y2 = acceptorPos[hoppingSite2,0],                                           acceptorPos[hoppingSite2,1]
                            

                    # ax.plot([x1,x2],[y1,y2],"-",alpha=0.05,color="k",linewidth=2)
                    ax.plot([x1,x2],[y1,y2],"-",alpha=0.05,color=color(l, len(electrodes)),linewidth=2)
