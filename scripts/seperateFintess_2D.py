...
for i in range(len(permutations)):
    counts2D.append(np.histogram2d(Delta[permutations[i][0]], Delta[permutations[i][1]], bins = bins, density = True)[0])

counts2D_linSeperated = []
for i in range(len(permutations)):
    counts2D_linSeperated.append(np.multiply( * np.meshgrid(Delta_counts[permutations[i][0]], Delta_counts[permutations[i][1]], indexing = "ij")))
    
for i in range(len(permutations)):
    counts2D[i]              *= (Delta_bins[permutations[i][0]][1]-Delta_bins[permutations[i][0]][0]) * (Delta_bins[permutations[i][1]][1]-Delta_bins[permutations[i][1]][0])
    counts2D_linSeperated[i] *= (Delta_bins[permutations[i][0]][1]-Delta_bins[permutations[i][0]][0]) * (Delta_bins[permutations[i][1]][1]-Delta_bins[permutations[i][1]][0])
...
for i in range(len(permutations)):
    ...
    im1 = ax1.imshow(counts2D[i]             , cmap = "gnuplot", vmin = vmin, vmax = vmax, extent = [ Delta_bins[permutations[i][1]][0], Delta_bins[permutations[i][1]][-1], Delta_bins[permutations[i][0]][-1], Delta_bins[permutations[i][0]][0]], aspect = "auto")
    im2 = ax2.imshow(counts2D_linSeperated[i], cmap = "gnuplot", vmin = vmin, vmax = vmax, extent = [ Delta_bins[permutations[i][1]][0], Delta_bins[permutations[i][1]][-1], Delta_bins[permutations[i][0]][-1], Delta_bins[permutations[i][0]][0]], aspect = "auto")
    ...
    plt.savefig(join(pathToSimFolder,f"density2D_Delta{permutations[i][0]+1}_Delta{permutations[i][1]+1}_bins_{bins}.png"),bbox_inches="tight",dpi=300)    
    # plt.show()
    plt.close(fig)
...
for gate in gates:
    ...
    for i in range(len(permutations)):
        ...
        fitness2D = np.histogram2d(Delta[permutations[i][0]], Delta[permutations[i][1]], bins = bins, weights = controlFitness)[0]/np.histogram2d(Delta[permutations[i][0]], Delta[permutations[i][1]], bins = bins)[0]

        vmin = min(np.min(fitness_seperated), np.min(fitness2D))
        vmax = max(np.max(fitness_seperated), np.max(fitness2D))

        im1 = ax1.imshow(fitness2D        , cmap = "gnuplot", vmin = vmin, vmax = vmax, extent = [ Delta_bins[permutations[i][1]][0], Delta_bins[permutations[i][1]][-1], Delta_bins[permutations[i][0]][-1], Delta_bins[permutations[i][0]][0]], aspect = "auto")
        im2 = ax2.imshow(fitness_seperated, cmap = "gnuplot", vmin = vmin, vmax = vmax, extent = [ Delta_bins[permutations[i][1]][0], Delta_bins[permutations[i][1]][-1], Delta_bins[permutations[i][0]][-1], Delta_bins[permutations[i][0]][0]], aspect = "auto")
        ...
        cbar_ax = fig.add_axes([0.86, 0.15, 0.04, 0.7])
        fig.colorbar(im1, cax=cbar_ax)
        plt.savefig(join(pathToSimFolder,f"{gate}_fitness2D_Delta{permutations[i][0]+1}_Delta{permutations[i][1]+1}_bins_{bins}.png"),bbox_inches="tight",dpi=300)    
        # plt.show()
        plt.close(fig)
