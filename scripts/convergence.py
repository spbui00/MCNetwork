...
if mode == "MC":
    ...
    for i in range(int(distance+meanRange/2),controlVoltages.shape[0]):
        mean = np.mean(controlVoltages[int(i-distance-meanRange/2):int(i-distance+meanRange/2), :],axis = 0)
    ...
    ax2=ax.twinx()
    ax2.plot(range(int(distance+meanRange/2),controlVoltages.shape[0]), displace, "k-",label="displacement")
...
if mode == "basinHop":
    ...
    for i in range(1,basinChanges.shape[1]):
        ax2.plot(np.arange(basinChanges[0,i-1],basinChanges[0,i]), np.sqrt(np.sum((controlVoltages[basinChanges[0,i-1]:basinChanges[0,i]] - basinBestVoltages)**2, axis = 1  )) ,color="k")
        # calc last basin best
        if basinChanges[1,i]:
            basinBestIdx      = np.argmax(optEnergy[basinChanges[0,i-1]:basinChanges[0,i]]) + basinChanges[0,i-1]
            basinBestVoltages = controlVoltages[basinBestIdx]
    ax2.plot(np.arange(basinChanges[0,-1],len(optEnergy)), np.sqrt(np.sum((controlVoltages[basinChanges[0,-1]:] - basinBestVoltages)**2, axis = 1  )) ,color="k")
