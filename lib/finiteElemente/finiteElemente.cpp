...
FiniteElementeBase::FiniteElementeBase(bool saveSolution):
...
FiniteElementeRect::FiniteElementeRect(double const & len, double const & width, int const & maxNumberOfElments, bool saveSolution):
...
void FiniteElementeRect::setElectrode(double const & voltage, double begin, double end, int edge ){
...
FiniteElementeRect::~FiniteElementeRect(){
...
FiniteElementeCircle::FiniteElementeCircle(double const & radius, int const & maxNumberOfElments, bool saveSolution):
                                                                        FiniteElementeBase(saveSolution),
                                                                        radius(radius){
    initMesh(maxNumberOfElments);
...
void FiniteElementeCircle::initMesh(int const & maxNumberOfElments){
    ...
    for(int l=layers; l > 1 ;l--){ //last layer extra
        ...
        while (true){
            ...
            // std::cout<<"out: "<<outerIndex<<" in: "<<innerIndex<<" tri: "<<triIndx[0]<<" "<<triIndx[1]<<" "<<triIndx[2]<<std::endl;
            mesh->AddTri(triIndx);
...
void FiniteElementeCircle::setElectrode(double const & voltage, double begin, double end){
    ...
    if (beginIdx<endIdx){
        ...
        electrodeVertexIndices.push_back(std::vector<int>(6*layers+endIdx-beginIdx));
        std::iota(electrodeVertexIndices.back().begin(), electrodeVertexIndices.back().begin()+6*layers-beginIdx, beginIdx);
        std::iota(electrodeVertexIndices.back().begin()+6*layers-beginIdx, electrodeVertexIndices.back().end(), 0);
    }
    ...
    // set boundary attribute to make (only) it mandatory
    for ( auto i = electrodeVertexIndices.back().begin(); i != electrodeVertexIndices.back().end()-1; i++ ) {
        mesh->GetBdrElement(*i)->SetAttribute(2);
    }
